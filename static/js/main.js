...
const PET_IMAGES = { "Яйце": "🥚", "Малий Демон": "🦇", "Демон-Вбивця": "🐺", "Вищий Демон": "👹", "Лорд Безодні": "👿", "Дракончик": "🦎", "Юний Дракон": "🐉", "Вогняний Дракон": "🔥", "Стародавній Дракон": "🐲", "Камінь": "🪨", "Малий Голем": "🗿", "Залізний Голем": "⚙️", "Сталевий Голем": "🛡️", "Титан": "🤖", "Ектоплазма": "💧", "Дух": "👻", "Полтергейст": "🌪️", "Жах": "😱", "Жнець Душ": "💀", "Попіл": "💨", "Іскра": "✨", "Вогняний Птах": "🕊️", "Фенікс": "🦅", "Безсмертний": "☀️", "Сліпе маля": "🐁", "Кажан": "🦇", "Кровопивця": "🩸", "Вампір": "🧛", "Носферату": "🧛‍♂️", "Цуценя": "🐶", "Вовк": "🐺", "Лютововк": "🐾", "Перевертень": "🐺", "Альфа": "👑", "Насіння": "🌱", "Саджанець": "🌿", "Ент": "🌳", "Древній Ент": "🌲", "Хранитель Лісу": "🌍" };
...
const ACH_DB = [ { id: "kills_10", name: "Перша кров", desc: "Вбити 10 ворогів", reqType: "kills", reqNum: 10, reward: 5 }, { id: "kills_50", name: "Кат", desc: "Вбити 50 ворогів", reqType: "kills", reqNum: 50, reward: 15 }, { id: "kills_200", name: "Жнець Душ", desc: "Вбити 200 ворогів", reqType: "kills", reqNum: 200, reward: 50 }, { id: "boss_1", name: "Падіння Слизу", desc: "Вбити 1 Боса", reqType: "boss_kills", reqNum: 1, reward: 10 }, { id: "boss_5", name: "Гроза Лордів", desc: "Вбити 5 Босів", reqType: "boss_kills", reqNum: 5, reward: 25 }, { id: "boss_20", name: "Вбивця Богів", desc: "Вбити 20 Босів", reqType: "boss_kills", reqNum: 20, reward: 100 }, { id: "map_10", name: "Мандрівник", desc: "Дійти до 10 етапу", reqType: "max_node", reqNum: 10, reward: 10 }, { id: "map_30", name: "Ветеран Безодні", desc: "Дійти до 30 етапу", reqType: "max_node", reqNum: 30, reward: 50 }, { id: "map_50", name: "Абсолют", desc: "Пройти гру (50 етап)", reqType: "max_node", reqNum: 50, reward: 500 } ];
const STORY_DB = { 5: { s: "Король Слизу", t: "Ви всі такі однакові... Приходите сюди за славою, а залишаєтесь кістками в моєму череві." }, 10: { s: "Забутий Лицар", t: "Ніхто не пройде мій міст!" }, 15: { s: "Матір Павуків", t: "Ти станеш чудовим інкубатором для моїх діточок." }, 20: { s: "Тінь Минулого", t: "Спробуй вбити власне сумління, якщо тобі вистачить духу!" }, 25: { s: "Генерал Пекла", t: "Клан Півночі впав від моєї руки, ти — наступний!" }, 30: { s: "Кривава Графиня", t: "Твоя життєва сила живитиме мою молодість ще сотню років." }, 35: { s: "Пожирач Снів", t: "Я змушу тебе пережити найглибші страхи в реальності!" }, 40: { s: "Падший Архангел", t: "Світло зрадило мене, і я дарую тобі вічну темряву." }, 45: { s: "Права рука Лорда", t: "За цією брамою кінець усього. Я не пропущу тебе." }, 50: { s: "АБСОЛЮТ ТІНІ", t: "ТЕПЕР Я ПОГЛИНУ ТЕБЕ, І ЦЕЙ СВІТ ЗГАСНЕ НАЗАВЖДИ!" } };
const ELIAS_LORE = [ "Ці землі колись були найродючішими в регіоні. Тепер Тьма спалила врожай.", "Я чув про могутній клан 'Орлів'... Вони впали, але їхня воля живе у таких воїнах, як ти.", "Деякі моби використовують хитрощі. Не піддавайся їхнім ілюзіям.", "Кажуть, Лорд Безодні був колись звичайною людиною...", "Тут були лицарські аванпости Північного Королівства. Тепер це лише братські могили.", "Твій Гримуар здатний на більше. Об'єднуй речі в Кузні." ];
const PET_PASSIVES_DB = { "crit": { name: "+10% Крит. шанс", color: "#ff3333" }, "vampire": { name: "+10% Вампіризм", color: "#1eff00" }, "poison": { name: "+10% Отрута", color: "#00ff00" }, "thorns": { name: "+10% Шипи", color: "#ff00ff" } };
...
function renderAchievements() {
    ...
    ACH_DB.forEach(a => {
        ...
        let btn = isCl ? `<button class="btn-small" style="background:#222; color:#555;" disabled>ОТРИМАНО</button>` : done ? `<button class="btn-small bg-gold" onclick="claimAch('${a.id}')">ЗАБРАТИ ${a.reward} 💎</button>` : `<span style="color:#aaa;">${cur}/${a.reqNum}</span>`;
...
function renderInventoryAndCraft() {
    let wAtk = 0; let aDef = 0; playerChar.crit_chance = 0.05; playerChar.vampire = 0; playerChar.poison = 0; playerChar.thorns = 0; playerChar.dodge = 0.05; 
...
function generateShop(category) {
    ...
    const SHOP_NAMES = { weapon: ["Меч Новачка", "Кривавий Кинджал", "Посох Мага", "Сокира Варвара", "Лук Ельфів", "Ефірний Клинок", "Коса Жреця", "Молот Титанів", "Спис Долі"], armor: ["Лляна Сорочка", "Шкіряна Броня", "Кольчуга Лицаря", "Сталевий Нагрудник", "Мантія Тіней", "Екзоскелет Безодні", "Плащ Асасина", "Щит Паладіна"] };
    let h = ''; let lvl = playerChar.level || 1;
    
    // Новий баланс: жорсткі множники залежно від ціни
    const rarityPool = [
        {r: 'Common', c: '#a0a0a0', m: 1.0, p: 100},
        {r: 'Uncommon', c: '#1eff00', m: 1.8, p: 400},
        {r: 'Rare', c: '#0070dd', m: 3.2, p: 1200},
        {r: 'Epic', c: '#a335ee', m: 5.5, p: 4500},
        {r: 'Legendary', c: '#ff8000', m: 9.0, p: 15000}
    ];

    rarityPool.forEach(t => {
        for(let i=0; i<2; i++) { // по 2 предмети кожної рідкості
            let name = SHOP_NAMES[category][Math.floor(Math.random() * SHOP_NAMES[category].length)];
            let base_stat = category === 'weapon' ? 15 : 10;
            let stat = Math.floor((lvl * base_stat) * t.m + Math.random() * 5);
            let icon = category === 'weapon' ? '⚔️' : '🛡️';
            
            h += `
            <div class="rng-item shop-card" style="border-left: 4px solid ${t.c}; width: 300px;" data-type="${category}">
                <div>
                    <strong style="color:${t.c};">${icon} ${name}</strong><br>
                    <span style="font-size:0.85rem; color:#aaa;">+${stat} ${category==='weapon'?'АТК':'ЗАХ'} | ${t.r}</span>
                </div>
                <button onclick="buyShopItem('${icon} ${name}', '${category}', '${t.r}', ${stat}, ${t.p})" class="btn-small bg-gold">Купити (${t.p} 🪙)</button>
            </div>`;
        }
    });
    container.innerHTML = h;
...
// 🔥 КУПІВЛЯ З ПИТАННЯМ ПРО ЕКІПІРУВАННЯ
async function buyShopItem(name, type, rarity, stat, price) { 
    const equipNow = confirm(`Бажаєте одягнути "${name}" відразу після покупки?`);
    ...
    if(r.ok) { showPopup("ПОКУПКА УСПІШНА", equipNow ? "Куплено та одягнено!" : d.message, "loot", "🛒"); loadPlayerData(); } 
...
function renderGachaAndPets() {
    ...
    document.getElementById('pet-list').innerHTML = p.length === 0 ? '<p style="color:#555;">Немає супутників.</p>' : p.map(pt => {
        let stg = pt.stage || 1; let p_html = (pt.passives||[]).map(psv => `<span style="background:${PET_PASSIVES_DB[psv].color}; color:#000; padding:2px 5px; border-radius:3px; font-size:0.7rem; font-weight:bold;">${PET_PASSIVES_DB[psv].name}</span>`).join(' ');
        let maxP = stg >= 20 ? 2 : (stg >= 10 ? 1 : 0); let canLearn = (pt.passives||[]).length < maxP;
...
function openPetSkillModal(petId) { activePetForSkill = petId; let h = ''; Object.keys(PET_PASSIVES_DB).forEach(k => { h += `<button class="btn-small" style="background:${PET_PASSIVES_DB[k].color}; color:#000; font-weight:bold; font-size:1.1rem; padding:10px;" onclick="learnPetPassive('${k}')">${PET_PASSIVES_DB[k].name}</button>`; }); document.getElementById('pet-skill-options').innerHTML = h; document.getElementById('pet-skill-modal').classList.remove('hidden'); }
function closePetSkillModal() { document.getElementById('pet-skill-modal').classList.add('hidden'); activePetForSkill = null; }

async function learnPetPassive(passiveId) { const r = await fetch('/api/learn_pet_passive', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('rpg_token')}` }, body: JSON.stringify({ pet_id: activePetForSkill, passive: passiveId }) }); const d = await r.json(); if(r.ok) { closePetSkillModal(); showPopup("НАВИЧКУ ЗАСВОЄНО", d.message, "loot", "🧬"); loadPlayerData(); } else { closePetSkillModal(); showPopup("Помилка", d.message, "loss", "❌"); } }
...
// 🔥 ОНОВЛЕНИЙ ЗАПИТ НА ДОСЯГНЕННЯ
async function claimAch(id) { const r = await fetch('/api/claim_achievement', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('rpg_token')}` }, body: JSON.stringify({ id }) }); const d = await r.json(); if(r.ok) { showPopup("ДОСЯГНЕННЯ!", `Ви отримали ${d.reward} 💎`, "loot", "🏆"); loadPlayerData(); } else showPopup("Зачекайте", d.message, "loss", "⏳"); }

function startStorySequence(n, b) { isSurvivalMode = false; playingNode = n; isBossNode = b; document.getElementById('story-overlay').classList.remove('hidden'); if(b && STORY_DB[n]) { document.getElementById('story-name').innerText = STORY_DB[n].s; document.getElementById('story-text').innerText = STORY_DB[n].t; document.getElementById('story-name').style.color = "var(--blood)"; document.getElementById('story-avatar').style.backgroundImage = `url('${BOSS_IMG}')`; } else { document.getElementById('story-name').innerText = "Провідник Еліас"; document.getElementById('story-text').innerText = ELIAS_LORE[(n-1) % ELIAS_LORE.length]; document.getElementById('story-name').style.color = "var(--gold)"; document.getElementById('story-avatar').style.backgroundImage = `url('https://images.unsplash.com/photo-1541604193435-22287d32c2c2?w=500')`; } }
...
function triggerBattle(n, b) { let scale = isSurvivalMode ? survivalWave : n; let eHp = Math.floor(120 + (scale * 55) * (b ? 3.5 : 1)); let eAtk = Math.floor(15 + (scale * 4.0) * (b ? 1.5 : 1)); currentEnemy = { name: isSurvivalMode ? `Тінь Хвилі ${survivalWave}` : (b ? (STORY_DB[n] ? STORY_DB[n].s : "ЛОРД БЕЗОДНІ") : "ПОРОДЖЕННЯ ТІНІ"), hp: eHp, maxHp: eHp, atk: eAtk, isBoss: b }; setupArena(); logBattle(`⚔️ Бій ${isSurvivalMode ? 'на Виживання' : 'на Етапі ' + n} почався!`, "gold"); }

function setupArena() {
    document.getElementById('game-screen').classList.add('hidden'); document.getElementById('battle-screen').classList.remove('hidden'); document.getElementById('b-e-name').innerText = currentEnemy.name; document.getElementById('b-e-hp').style.width = '100%'; document.getElementById('b-p-hp-bar').style.width = '100%'; document.getElementById('b-p-card').style.backgroundImage = `url('${AVATARS[playerChar.class_name] || AVATARS['Воїн']}')`; document.getElementById('enemy-card').style.backgroundImage = currentEnemy.isBoss ? `url('${BOSS_IMG}')` : "url('https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500')"; document.getElementById('b-log').innerHTML = ''; playerChar.hp = playerChar.max_hp; 
    let aPet = playerChar.pets ? playerChar.pets.find(p => p.id === playerChar.active_pet) : null; let petEl = document.getElementById('b-p-pet'); if(aPet) { petEl.innerText = PET_IMAGES[aPet.name] || '🥚'; petEl.style.display = 'block'; } else { petEl.style.display = 'none'; }
    let eqS = getCleanSkills(); skillCooldowns = {}; eqS.forEach(k => skillCooldowns[k] = 0); renderBattleSkills(); if(battleTimer) clearInterval(battleTimer);
    battleTimer = setInterval(() => { 
        let changed = false; Object.keys(skillCooldowns).forEach(k => { if(skillCooldowns[k] > 0) { skillCooldowns[k]--; changed = true; } }); 
        if(playerChar.poison > 0 && currentEnemy.hp > 0) { let psnDmg = Math.floor(playerChar.totalAtk * playerChar.poison); currentEnemy.hp -= psnDmg; spawnDamageText('enemy-card', psnDmg, "psn"); document.getElementById('b-e-hp').style.width = Math.max(0, (currentEnemy.hp / currentEnemy.maxHp)*100) + '%'; if(currentEnemy.hp <= 0) { clearInterval(battleTimer); finishBattle(true); return; } }
        if(changed) renderBattleSkills(); 
    }, 1000);
}

function renderBattleSkills() { let h = ''; let eqS = getCleanSkills(); let unlS = playerChar.unlocked_skills || {}; eqS.forEach(k => { const s = SKILLS_DB[k]; if(!s) return; const cd = skillCooldowns[k] || 0; let lvl = unlS[k] || 1; h += `<div class="skill-btn ${cd <= 0 ? '' : 'on-cd'}" onclick="useSkill('${k}', ${lvl})"><div class="skill-icon">${s.icon}</div><div class="skill-name">${s.name}</div>${cd > 0 ? `<div class="cd-overlay">${cd}s</div>` : ''}</div>`; }); document.getElementById('battle-skills').innerHTML = h; }

async function useSkill(k, lvl) {
    if (skillCooldowns[k] > 0 || !currentEnemy || currentEnemy.hp <= 0) return; const s = SKILLS_DB[k]; animateElement('b-p-card', 'attack-bounce'); if(document.getElementById('b-p-pet').style.display !== 'none') { animateElement('b-p-pet', 'attack-bounce'); }
    let isCrit = Math.random() < playerChar.crit_chance; let dynamicMult = s.mult + (lvl * 0.2); let dmg = Math.floor(playerChar.totalAtk * dynamicMult * (isCrit ? 2.0 : 1.0));
    currentEnemy.hp -= dmg; spawnDamageText('enemy-card', dmg, isCrit ? "crit" : "normal"); if(isCrit) screenShake();
    if(playerChar.vampire > 0 || s.type === "vampire") { let heal = Math.floor(dmg * (playerChar.vampire + (s.type === "vampire" ? 0.3 : 0))); playerChar.hp = Math.min(playerChar.max_hp, playerChar.hp + heal); spawnDamageText('b-p-card', `+${heal}`, "heal"); document.getElementById('ui-hp-bar').style.width = Math.max(0, (playerChar.hp / playerChar.max_hp)*100) + '%'; document.getElementById('b-p-hp-bar').style.width = Math.max(0, (playerChar.hp / playerChar.max_hp)*100) + '%'; }
    ...
    if(s.cd > 0) { skillCooldowns[k] = s.cd; renderBattleSkills(); } if (currentEnemy.hp <= 0) { clearInterval(battleTimer); finishBattle(true); return; }
    setTimeout(() => {
        if(currentEnemy.hp <= 0) return; let isDodge = Math.random() < playerChar.dodge;
        if(isDodge) { logBattle(`💨 Ухилення!`, "cyan"); spawnDamageText('b-p-card', "MISS", "miss"); } else { 
            let eDmg = Math.max(1, currentEnemy.atk - Math.floor(playerChar.totalDef * 1.5)); playerChar.hp -= eDmg; animateElement('enemy-card', 'attack-bounce-reverse'); spawnDamageText('b-p-card', eDmg, "enemy"); document.getElementById('ui-hp-bar').style.width = Math.max(0, (playerChar.hp / playerChar.max_hp)*100) + '%'; document.getElementById('b-p-hp-bar').style.width = Math.max(0, (playerChar.hp / playerChar.max_hp)*100) + '%'; screenShake(5); logBattle(`Ворог б'є: -${eDmg} HP`, "red"); 
            if(playerChar.thorns > 0) { let thrnDmg = Math.floor(eDmg * playerChar.thorns); currentEnemy.hp -= thrnDmg; spawnDamageText('enemy-card', thrnDmg, "thrn"); logBattle(`Шипи: ворог отримує ${thrnDmg}`, "purple"); document.getElementById('b-e-hp').style.width = Math.max(0, (currentEnemy.hp / currentEnemy.maxHp)*100) + '%'; }
        }
        if (playerChar.hp <= 0) { clearInterval(battleTimer); finishBattle(false); } if (currentEnemy.hp <= 0) { clearInterval(battleTimer); finishBattle(true); return; }
    }, 1000);
}

// 🔥 ОНОВЛЕНА ЛОГІКА ЗАВЕРШЕННЯ БОЮ
async function finishBattle(w) {
    if (isSurvivalMode) {
        ...
        const d = await r.json();
        if (r.ok) { let msg = `Отримано: +${d.gold} 🪙`; if(d.chests) msg += `<br><span style="color:var(--gold)">🎁 Ви знайшли скриню!</span>`; showPopup("ХВИЛЯ ПРОЙДЕНА!", msg, "win", "🛡️"); survivalWave++; setTimeout(() => { closePopup(); triggerBattle(survivalWave, survivalWave % 5 === 0); }, 1500); }
        return;
    }
...
function spawnDamageText(id, txt, type) { const t = document.getElementById(id); const f = document.createElement('div'); f.className = `floating-text ${type}`; f.innerText = txt; if(type === 'psn') { f.style.color = '#00ff00'; f.style.textShadow = '0 0 5px #00ff00'; } if(type === 'thrn') { f.style.color = '#ff00ff'; f.style.textShadow = '0 0 5px #ff00ff'; } t.appendChild(f); setTimeout(() => f.remove(), 800); }
